...
create_storage_account() {
    ...
    if ! az storage account show --name "$acct" --resource-group "$rg_group" &>/dev/null;
    then 
...
create_container() {
    ...
    az storage container create --name "$cont_name" --account-name "$strg_acct_name" --auth-mode login
    fi
}
